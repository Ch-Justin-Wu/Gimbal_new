...
static struct_pid imu_temp_pid;
// 加速度计低通滤波
...
void BMI_update(fp32 *yaw, fp32 *pitch)
{
    //*yaw += bmi088_real_data.gyro[0]*0.001f;
    *pitch += bmi088_real_data.gyro[1] * 0.001f;
}
void AHRS_update(fp32 quat[4], fp32 time, fp32 gyro[3], fp32 accel[3], fp32 mag[3])
{
...
void IMU_Init(void)
{
    ...
    pid_param_init(&imu_temp_pid);
    imu_temp_pid.init(&imu_temp_pid, imu_temp_PID[3], imu_temp_PID[4], 0.0f, 0, imu_temp_PID[0], imu_temp_PID[1], imu_temp_PID[2], 0);
...
static void imu_temp_control(fp32 temp)
{
    ...
    if (first_temperate)
    {
        ...
        imu_temp_pid.calc(&imu_temp_pid, temp, 45.0f);
